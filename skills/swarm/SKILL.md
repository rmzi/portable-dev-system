...
1. Create the team:
   ...
   ```
2. Spawn workers via `Task(worker)` — use the typed syntax to enforce agent type restrictions. Workers declare `isolation: worktree` in their frontmatter; Claude Code (since 2.1.50) provisions the worktree automatically. No manual worktree creation needed:
   ```
...
2. Spawn a reviewer for pre-human code review:
   ```
   Task(reviewer, prompt="Review the diff against acceptance criteria from Phase 1.")
   ```
...
4. Spawn a documenter if user-facing docs are affected:
   ```
   Task(documenter, prompt="Update docs for the changes in this PR.")
   ```
...
1. Spawn scout for PDS meta-improvements:
   ```
   Task(scout, prompt="Read .claude/instincts.md. Update counts for re-observed patterns. Propose new instincts. Flag high-confidence patterns for skill promotion. Run /pds:eval on skills exercised in this swarm. Write report to .claude/swarm/scout-report.md.")
   ```
