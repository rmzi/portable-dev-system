...
Agents coordinate via Claude Code's native team tools — TaskCreate/TaskUpdate/TaskList/TaskGet, SendMessage, TaskStop. Team formation and cleanup are automatic (Claude Code v2.1.178+) — there is no `TeamCreate`/`TeamDelete` call; a team forms on the first teammate spawn and tears down when the session ends. See each agent's frontmatter `tools:` field for which tools it has access to. Claude Code's built-in tool documentation covers usage, protocols (shutdown, plan approval, idle state, messaging).
