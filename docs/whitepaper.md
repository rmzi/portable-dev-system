...
**Known gap:** PDS uses 7 of 28 available hook events. High-value unused events include: PreToolUse (gate tool calls with PDS policy beyond phase gates), PostToolUse (usage telemetry and pattern tracking), SubagentStart (enforce agent roster — warn on unknown types), and UserPromptSubmit (suggest relevant PDS skills based on user prompt keywords). **Path forward:** Prioritize PostToolUse for telemetry and SubagentStart for roster enforcement as the highest-impact additions.
