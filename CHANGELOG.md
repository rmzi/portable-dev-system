...
- **`git`/`gh` failed under the sandbox despite being `excludedCommands`, surfaced while pushing this very branch.** `git fetch`/`push` against an SSH remote (`git@github.com:...`) failed with an SSH negotiation error, and `gh` failed to read its keyring token — both while sandboxed, both working fine with the sandbox disabled. Root cause, confirmed against Claude Code's sandboxing docs: `excludedCommands` exempts a command from filesystem confinement, but the sandbox's network proxy is HTTP(S)-only and does not tunnel raw SSH even to an allowed domain, and keychain/credential-store access under the sandbox is undocumented and empirically unreliable. This is a Claude Code platform gap, not something `.claude/settings.json` can configure around. `session-start.sh` now warns once at session start if `origin` is an SSH remote, naming the workaround (switch to HTTPS with a credential helper, or disable the sandbox for that command) instead of letting it surface as a confusing mid-task failure. `docs/teams.md`'s sandbox section corrected to state this caveat rather than implying full bypass.
...
- **Not yet exercised**: a full live swarm run through Phase 6 to observe the new `Stop`-based teardown gate fire and block/allow correctly under real conditions, and a live `/pds:resume` run against all three of its stated scenarios (same-machine, cross-machine via ticket, zero-local-state via label). Both mechanisms are built and doc-verified against Claude Code's current hooks reference, not yet runtime-verified.
