...
### Fixed
- **#181 — the orchestrator could not spawn any agent at all; `/pds:swarm` was dead end-to-end.** `agents/orchestrator.md` declared its spawn allowlist with bare agent names — `Task(researcher, worker, validator, reviewer, documenter, scout, auditor)` — but PDS ships its agents inside a plugin, and plugin-provided agents register under the plugin namespace as `pds:<name>`. The allowlist therefore matched zero agents, and Claude Code's response to a zero-match allowlist is to empty the spawn roster *entirely*: every subsequent spawn failed with `Agent type '<x>' not found. Available agents:` and nothing after the colon — `general-purpose` included. That trailing empty list is the tell, and it made the failure read as "the plugin didn't load" rather than "the allowlist is misnamed," which is why it survived this long. Fixed by namespacing every entry (`Task(pds:researcher, pds:worker, ...)`).
- **#181 (secondary) — `pds:shepherd` was absent from the orchestrator allowlist entirely.** `/pds:swarm` spawns a shepherd at med/heavy tier in Phase 1 and re-checks in Phase 3, so that spawn would have failed even once the namespace prefix was correct. Added to the allowlist.
- Propagated the `pds:` prefix through every spawn example that prescribed the broken form: `skills/swarm/SKILL.md` (Phases 1, 3, 4, 5, 6), `skills/team/SKILL.md`, `docs/architecture.md`, `docs/teams.md`, `docs/whitepaper.md`. Each site also now states the rule and its failure signature, including the one real exception — project-scope agents under `.claude/agents/` resolve bare, with no prefix. `skills/swarm/SKILL.md`'s two-phase delegation block already used `Agent(subagent_type="pds:orchestrator")` correctly; the correct form was known and simply never reached the agent definition.

- **#181 (third site) — the `SubagentStart` roster-check hook matched bare names only, inverting its own signal.** `hooks/scripts/roster-check.sh` compared `agent_type` against `orchestrator|worker|validator|...`, but live spawns report `pds:<name>` — confirmed straight from a captured hook payload (`{"agent_type":"pds:orchestrator", ...}`). So it emitted `[PDS] Warning: unknown agent type 'pds:worker'` on *every legitimate spawn* while staying silent on genuinely unknown ones. It exits 0 unconditionally, so the cost was noise rather than broken dispatch — but it is the same root cause as the allowlist bug, sitting undetected in a second file. It also predated the shepherd and never listed it. Now strips the namespace before matching and derives the roster from the plugin's own `agents/` directory when `CLAUDE_PLUGIN_ROOT` resolves, so adding an agent needs no edit; the static list remains as a fallback for direct invocation. New `hooks/tests/test-roster-check.sh` (20 cases) covering namespaced and bare forms, built-ins, unknown types, the `shared-rules` mixin, and the never-blocks guarantee.
- **Roster size was documented as 8 across five files for a 9-agent roster.** `README.md`, `docs/teams.md`, `docs/whitepaper.md`, `docs/claude-code-extension-catalog.md`, and `docs/claude-code-source-analysis.md` all omitted the shepherd. Filed under Fixed rather than Docs because the same agent that was invisible in the prose was the one missing from the orchestrator's spawn allowlist and from the roster-check hook — a stale count is how a whole agent stays unnoticed. `scripts/check-agent-roster.py` now cross-checks documented counts against `agents/` on disk, so the number cannot drift again.

- **#182 — `pds:worker` was still unspawnable; the #170 fix was built on a wrong reading of the `WorktreeCreate` contract.** Registering a `WorktreeCreate` hook *replaces* Claude Code's native git worktree creation — the hook is not notified that a worktree was made, it is the thing that makes it. Measured against Claude Code 2.1.221 with an instrumented hook: it fires with CWD set to the **main repo**, before any worktree exists, and passes `{"name":"agent-<id>","cwd":"<repo>", ...}` on stdin, expecting the created worktree's absolute path on stdout. v5.0.0 closed #170 by printing `$(pwd)` behind a guard requiring `git rev-parse --git-dir` to contain `worktrees` — a condition that never holds, because CWD is the main repo and `--git-dir` is plain `.git`. The script therefore exited 0 with no stdout on every real firing, which is exactly the reported "hook succeeded but returned no worktree path". `sync-worktree-permissions.sh` now parses the payload, resolves the repo root via `--git-common-dir`, creates the worktree at `$REPO_ROOT/.worktrees/$name` per the in-repo hygiene rule, prints the path, and only then attempts the `settings.local.json` symlink as best-effort work that can never suppress the path. Verified live: `pds:worker` spawns and returns from a real repo. Note the hook had never done anything but symlink — it was registered as a side-effect script and, purely by being registered, silently took worktree creation away from Claude Code and then created nothing.
- **Missing `WorktreeRemove` counterpart.** Owning creation means owning teardown; without it every spawned worker leaked a worktree into `.worktrees/`, which also trips the Phase 6 teardown gate's clean-worktree check. New `hooks/scripts/remove-agent-worktree.sh`, registered in `hooks/hooks.json`. It refuses any path outside `$REPO_ROOT/.worktrees/` — the payload is untrusted input and `git worktree remove --force` deletes files — and exits 0 on every path so a failed cleanup can never fail a session. `WorktreeCreate`'s timeout was raised from 5s to 30s, since it now performs a `git worktree add` rather than a symlink.

### Added
- `scripts/check-workflows.py` — validates GitHub Actions workflow files. Added after a step named `- name: SubagentStart roster check handles pds: namespace` was committed during this very changeset: the colon+space ends a plain YAML scalar, the file stopped parsing, and GitHub reported only "This run likely failed because of a workflow file issue" with no line number and no job list. Every job was skipped, so the branch's check list went empty rather than red — a typo inside a newly added regression guard silently disabled the entire suite. Does a real parse when PyYAML happens to be importable and otherwise falls back to a dependency-free lint for that exact failure class, per the Portability of Operation principle; PyYAML is never required, keeping the markdown/bash/python3/jq contract intact.
- `scripts/smoke-dispatch.sh` — **live** dispatch smoke test, and the direct answer to "how do we never regress this again." It drives a real headless orchestrator (`claude -p --agent pds:orchestrator --output-format stream-json`) against a throwaway git repo and asserts that agent types actually spawn, reading verdicts out of the raw tool results rather than trusting the model's narration. Default run probes `pds:researcher` (plain spawn path) and `pds:worker` (worktree path), which between them cover both historical failure modes; `--all` probes every spawnable type. Failure output names the specific cause — unresolved type, denied type, worktree-create failure — and points at the file to check. It needs an authenticated `claude` and so cannot run in CI; `/pds:finish` step 4 and `/pds:contribute` step 2 now require a human to run it whenever a branch touches `agents/`, `hooks/`, or spawn syntax in `skills/`.
- `hooks/tests/test-worktree-hooks.sh` — 12 cases covering the `WorktreeCreate`/`WorktreeRemove` contract: path is printed and correct, worktree lands under `.worktrees/`, idempotent on repeat fire, symlink performed, path still printed when there's no `settings.local.json` to link, empty payload exits silently, non-git cwd fails loudly instead of returning nothing, removal by `worktreePath` and by `name` alone, and refusal to delete outside `.worktrees/`. Includes an explicit named guard against #170's exact shape — exit 0 with empty stdout on a valid payload.
- New `dispatch-contract` CI job running the roster check, the worktree hook tests, and the full `install.sh --test` suite on every push and PR.
- **"Dispatch Is Load-Bearing" section in `CLAUDE.md`**, plus the namespacing rule and its failure signature. Two releases have now shipped with zero working dispatch and a fully green test suite, because in both cases the violated contract was Claude Code's rather than PDS's. A green static suite is necessary and never sufficient.
- `scripts/check-agent-roster.py` — regression guard for #181, wired into `install.sh --test`. Asserts that the orchestrator's `Task(...)` allowlist is `pds:`-namespaced, that it names no agent lacking a definition, and that it covers every agent in `agents/` except `orchestrator` itself and the non-spawnable `shared-rules` mixin. Verified against both historical failure shapes: it rejects the bare-name allowlist and it rejects a namespaced allowlist with `shepherd` missing.

### Notes
- **Relationship to #171.** #171 diagnosed the same visible symptom one layer too high. It concluded that named-teammate nesting was blocking dispatch and removed `name=` from every spawn call. Spawns were failing before ever reaching that check — the agent type never resolved. #171's change remains correct on its own terms; it just never restored dispatch, and the roster stayed empty.
- Root cause was isolated with headless probes (`claude -p --output-format stream-json`) rather than model narration, comparing plugin-scope against project-scope agents under unfiltered `Task`, namespaced `Task(pds:...)`, and bare `Task(...)` allowlists. The parameterized `Task(a, b)` frontmatter syntax itself works correctly and was never at fault.
